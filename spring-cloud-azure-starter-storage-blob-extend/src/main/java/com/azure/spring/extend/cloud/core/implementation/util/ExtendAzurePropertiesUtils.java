...
package com.azure.spring.extend.cloud.core.implementation.util;
 ...
 *
 */
public final class ExtendAzurePropertiesUtils {

    private static final Logger LOGGER = LoggerFactory.getLogger(ExtendAzurePropertiesUtils.class);

    private ExtendAzurePropertiesUtils() {
     ...
     * @param <T> The type of the target that extends AzureProperties.
     */
    public static <T extends AzureProperties> void copyAzureCommonPropertiesIgnoreTargetNull(AzureProperties source, T target) {
        copyPropertiesIgnoreTargetNull(source.getClient(), target.getClient());
        copyHttpLoggingProperties(source, target, true);

        copyPropertiesIgnoreTargetNull(source.getProxy(), target.getProxy());
        copyPropertiesIgnoreTargetNull(source.getProfile(), target.getProfile());
        copyPropertiesIgnoreTargetNull(source.getProfile().getEnvironment(), target.getProfile().getEnvironment());
        copyPropertiesIgnoreTargetNull(source.getCredential(), target.getCredential());

        if (source instanceof RetryOptionsProvider && target instanceof RetryOptionsProvider) {
            RetryOptionsProvider.RetryOptions sourceRetry = ((RetryOptionsProvider) source).getRetry();
            RetryOptionsProvider.RetryOptions targetRetry = ((RetryOptionsProvider) target).getRetry();
            copyPropertiesIgnoreTargetNull(sourceRetry, targetRetry);
            copyPropertiesIgnoreTargetNull(sourceRetry.getExponential(), targetRetry.getExponential());
            copyPropertiesIgnoreTargetNull(sourceRetry.getFixed(), targetRetry.getFixed());
        }
    }
    ...
    /**
     * Copy common properties from source object to target object. Ignore the target value if it is null.
     *
     * @param source The source object.
     * @param target The target object.
     */
    ...
    public static String getStorageAccountName(AzureStorageBlobProperties blobProperties) {
        String accountName = blobProperties.getAccountName();
        if (!StringUtils.hasText(accountName) && StringUtils.hasText(blobProperties.getEndpoint())) {
            try {
                BlobUrlParts parts = BlobUrlParts.parse(new URL(blobProperties.getEndpoint()));
                accountName = parts.getAccountName();
            } catch (MalformedURLException e) {
                LOGGER.error("Get storage account exception.", e);
            }
        }
        if (!StringUtils.hasText(accountName) && StringUtils.hasText(blobProperties.getConnectionString())) {
            ClientLogger clientLogger = new ClientLogger(ExtendAzurePropertiesUtils.class);
            StorageConnectionString connectionString = StorageConnectionString.create(blobProperties.getConnectionString(), clientLogger);
            accountName = connectionString.getAccountName();
        }

        if (StringUtils.hasText(accountName)) {
            accountName = accountName.toLowerCase(Locale.ROOT);
        }
        return accountName;
    }
